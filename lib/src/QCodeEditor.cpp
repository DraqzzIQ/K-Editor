...
QCodeEditor::QCodeEditor(QWidget* widget) :
    ...
    m_addressArea(new QAddressArea(this)), m_completer(nullptr), m_framedAttribute(new QFramedTextAttribute(this)),
    m_autoIndentation(true), m_autoParentheses(true), m_replaceTab(true), m_tabReplace(QString(4, ' ')),
    m_instructionSize(0)
{
...
{
    connect(document(), &QTextDocument::blockCountChanged, this, &QCodeEditor::updateViewportMargins);

    connect(verticalScrollBar(), &QScrollBar::valueChanged, [this](int) { m_lineNumberArea->update(); });

    connect(verticalScrollBar(), &QScrollBar::valueChanged, [this](int) { m_addressArea->update(); });

    connect(this, &QTextEdit::textChanged, [this] { m_addressArea->update(); });
...
void QCodeEditor::setHighlighter(QRiscvAsmHighlighter* highlighter)
{
    if (m_highlighter) {
...
void QCodeEditor::setSyntaxStyle(QSyntaxStyle* style)
{
    ...
    m_addressArea->setSyntaxStyle(m_syntaxStyle);
...
void QCodeEditor::updateLineGeometry()
{
    ...
    m_addressArea->setGeometry(
        QRect(cr.right() - m_addressArea->sizeHint().width() - verticalScrollBar()->sizeHint().width(), cr.top(),
              m_addressesVisible ? m_addressArea->sizeHint().width() : 0, cr.height()));
}

void QCodeEditor::updateViewportMargins(int)
{
    setViewportMargins(m_lineNumberArea->sizeHint().width(), 0,
                       m_addressesVisible ? m_addressArea->sizeHint().width() : 0, 0);
}
...
void QCodeEditor::updateAddressArea(const QRect& rect)
{
    m_addressArea->update(viewport()->width(), rect.y(), m_addressArea->sizeHint().width(), rect.height());
}
...
{
    updateAddressArea(e->rect());
    updateLineNumberArea(e->rect());
    updateLineGeometry();

    if (e->rect().contains(viewport()->rect())) {
        updateViewportMargins(0);
    }
...
void QCodeEditor::setAutoIndentation(bool enabled) { m_autoIndentation = enabled; }

bool QCodeEditor::autoIndentation() const { return m_autoIndentation; }

void QCodeEditor::setAutoParentheses(bool enabled) { m_autoParentheses = enabled; }
...
void QCodeEditor::setAddressesVisible(const bool visible)
{
    m_addressesVisible = visible;
    updateLineGeometry();
    updateViewportMargins(0);
}
...
void QCodeEditor::insertFromMimeData(const QMimeData* source) { insertPlainText(source->text()); }
